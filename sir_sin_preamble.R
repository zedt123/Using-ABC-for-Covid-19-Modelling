...
SIR <- function(t,x, parameters){
  ...
  with(as.list(parameters),{
    dS <- -(alpha_1 + alpha_2 * cos(alpha_3*t)) * S * I / N
    dI <- (alpha_1 + alpha_2 * cos(alpha_3*t)) * S * I / N - gamma * I
    dR <- gamma * I
    res <- c(dS, dI, dR)
    list(res)
  })
...
run_model <- function(S0, I0, R0, alpha_1, alpha_2, alpha_3, gamma, t_total){
  out <- lsoda(func = SIR,
               y = c(S = S0, I = I0, R = R0),
               parms = c(alpha_1=alpha_1, alpha_2=alpha_2, alpha_3=alpha_3, gamma = gamma),
