...
data$Dw <- c(0,data$D[2:1008] - data$D[1:1007])
data$Dw <- zoo::rollsum(data$Dw,7, fill = 0, align = 'right')
data$D <- data$D - data$Dw # ensure the sum of all compartments is the same
...
write.csv(data,"../Data/england_BSIRD_data.csv")
